...
public class MessageTemplateController extends BaseCrudRestController {
    ...
    public JsonResult list(MessageTemplate messageTemplate, Pagination pagination, HttpServletRequest request) throws Exception {
        //构建查询条件
        QueryWrapper<MessageTemplate> queryWrapper = super.buildQueryWrapper(messageTemplate, request);
        // 查询当前页的Entity主表数据
     ...
     */
    @PostMapping("/")
    public JsonResult createEntity(@ModelAttribute MessageTemplate entity, HttpServletRequest request)
            throws Exception{
     ...
     */
    @PutMapping("/{id}")
    public JsonResult updateModel(@PathVariable("id")Long id, @ModelAttribute MessageTemplate entity, HttpServletRequest request) throws Exception{
